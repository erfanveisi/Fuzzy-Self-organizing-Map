...
disp(size(find(sum(y==ooi) ~= 3),2));
%%
figure(3)
subplot(1,3,1)
plot(data(1,1:50),data(2,1:50),'r.','MarkerSize',18)
hold on
plot(data(1,51:100),data(2,51:100),'g.','MarkerSize',18)
hold on
plot(data(1,101:150),data(2,101:150),'b.','MarkerSize',18)
title('Orginal Data');
...
for i=1:L
    datai=data(ooi==i,:);
    plot(datai(:,1),datai(:,2),'.','MarkerSize',18,'Color',Colors(i,:));
    hold on
    title('Crisp Membership');
end
%%
Colors=hsv(L);
subplot(1,3,3)
for i=1:size(data,1)
    ci=oo(:,i)';
    ci=ci/max(ci);
    plot(data(i,1),data(i,2),'.','MarkerSize',18,'Color',ci);
    hold on;
    title('Fuzyy Membership');
end
